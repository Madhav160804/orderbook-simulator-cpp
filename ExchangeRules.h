...
struct ExchangeRules {
    ...
    void SetTickSize(Price t) {
        if (t <= 0) throw std::invalid_argument("tickSize must be positive");
        tickSize = t;
    }

    void SetLotSize(Quantity l) {
        if (l == 0) throw std::invalid_argument("lotSize must be positive");
        lotSize = l;
    }
